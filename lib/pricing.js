...
const CURRENCY_CONFIG = {
  USD: { symbol: '$', code: 'USD', locale: 'en-US', westernPremium: true },
  INR: { symbol: '₹', code: 'INR', locale: 'en-IN', westernPremium: false },
  GBP: { symbol: '£', code: 'GBP', locale: 'en-GB', westernPremium: true },
  AED: { symbol: 'د.إ', code: 'AED', locale: 'ar-AE', westernPremium: true },
};
...
/**
 * Apply Western Premium rounding (e.g., $X.99) for luxury pricing
 * Only applies to currencies where westernPremium is true
 * @param {number} price - Price in target currency
 * @param {string} currency - Target currency code
 * @returns {number} Adjusted price
 */
function applyWesternPremium(price, currency) {
  const config = CURRENCY_CONFIG[currency];
  if (!config || !config.westernPremium) return price;
  
  // If price is less than 1, keep as is (e.g., $0.78)
  if (price < 1) return Math.round(price * 100) / 100;
  
  // Round down to nearest whole number and add 0.99
...
export function convertPerMinute(baseINR, currency) {
  ...
  // Apply Western Premium rounding for luxury pricing
  const premium = applyWesternPremium(converted, currency);
